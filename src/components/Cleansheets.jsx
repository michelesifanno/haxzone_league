...
export default function Cleansheets() {
    ...
    const bestGoalkeeperData = playerStats.reduce((prev, current) => (prev.stats.cleanSheets > current.stats.cleanSheets) ? prev : current);
    const bestGoalkeeper = players.find((player) => player.name === bestGoalkeeperData.name);


    const bestGoalkeeperTeam = teams.find((team) => team.name === bestGoalkeeper.team.name);

    const logoStyle = isMobile ? { width: 120, marginRight: '5px' } : { width: 120, marginRight: 10 };

    playerStats.sort((a, b) => {
        // Ordina per punti decrescenti
        if (a.stats.cleanSheets !== b.stats.cleanSheets) {
            return b.stats.cleanSheets - a.stats.cleanSheets;
        }
    });

    const filteredPlayerStats = playerStats.filter(player => player.name !== bestGoalkeeper.name);
            ...
            <Grid container spacing={2} justifyContent="flex-start" alignItems="center">
                <Grid item xs={12} alignItems="center">
                    ...
                    <Typography variant="p" component="p" color={theme.palette.text.primary}>
                        Di seguito, la classifica Clean Sheets... Finora!
                    </Typography>
                </Grid>
            </Grid>
