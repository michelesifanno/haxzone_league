...
export default function HattrickMan() {
    ...
    const mostHatTricksPlayerData = playerStats.reduce((prev, current) => (prev.stats.hatTricks > current.stats.hatTricks) ? prev : current);
    const mostHatTricksPlayer = players.find((player) => player.name === mostHatTricksPlayerData.name);

    const mostHatTricksPlayerTeam = teams.find((team) => team.name === mostHatTricksPlayer.team.name);
    ...
    playerStats.sort((a, b) => {
        ...
        if (a.stats.hatTricks !== b.stats.hatTricks) {
            return b.stats.hatTricks - a.stats.hatTricks;
        }
    });

    const filteredPlayerStats = playerStats.filter(player => player.name !== mostHatTricksPlayer.name);
    ...
    return (
        <>
            <Grid container spacing={2} justifyContent="flex-start" alignItems="center">
                <Grid item xs={12} alignItems="center">
                    ...
                    <Typography variant="p" component="p" color={theme.palette.text.primary}>
                        Di seguito, la classifica delle Triplette... Finora!
                    </Typography>
                </Grid>
            </Grid>
            <Grid container spacing={2} justifyContent="flex-start" alignItems="center" className='matchGrid' sx={{ backgroundColor: '#fbfbfd', padding: '30px', border: '1px solid #eee', margin: '20px 0px' }}>
                ...
                <Link
                            to={`/team/${mostHatTricksPlayerTeam.logo}`}
                            style={{
                        ...
                        >
                            <img src={mostHatTricksPlayerTeam.logo} alt="Logo Team" style={logoStyle} />
                    </Link>
                </Grid>
                <Grid xs={12} md={6} alignItems="center" sx={{ padding: '10px' }}>
                <Link
                                                to={`/player/${mostHatTricksPlayer}`}
                                                style={{
